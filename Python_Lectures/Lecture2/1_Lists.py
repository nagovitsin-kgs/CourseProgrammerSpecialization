...
# Мы можем перебирать элементы списка с помощью циклов for и while (ниже более подробно про них!)
...
# ОПЕРАТОР ПРЕРЫВАНИЯ в python — break (перерыв, разрыв)
...
# ОПЕРАТОР ПРОПУСКА python — continue (продолжить)
...
# ИТЕРАЦИЯ по спискам с ФУНКЦИЕЙ range() (в диапазоне)

# Если вам нужно получить доступ к индексам списка, не очевидно как использовать цикл for для этой задачи.
# Мы можем получить доступ ко всем элементам, но индекс элемента остается недоступным.
# Есть способ получить доступ как к индексу элемента, так и к самому элементу. Для этого используйте функцию range() в сочетании с функцией длины len() :
...
# Чтобы наглядно увидеть, что может случиться, посмотрите на следующий пример :
colours = ["красный"]
...
for i in colours:
    ...
    if i == "черный":
        colours += ["белый"]
print(colours)  # ['красный', 'черный', 'белый']
...
# ФУНКЦИЯ Enumerate (перечислять, точно подсчитывать) в python 3

# Enumerate — встроенная функция Python.
# Большинство новичков и даже некоторые продвинутые программисты не знают о ней.
# Она позволяет нам АВТОМАТИЧЕСКИ СЧИТАТЬ ИТЕРАЦИИ ЦИКЛА.

# Вот пример :(enumerate - точно подсчитать, some_list - какой-то список, counter - счётчик, value - значение)

some_list = [1, 2, 3, 'слово', 10, 20]
