"""
Задача 6: Вы пользуетесь общественным транспортом?
Вероятно, вы расплачивались за проезд и получали билет с номером.
Счастливым билетом называют такой билет с шестизначным номером, где
сумма первых трех цифр равна сумме последних трех.
Т.е. билет с номером 385916 – счастливый, т.к. 3+8+5=9+1+6.
Вам требуется написать программу, которая проверяет счастливость билета.
Пример:
385916 -> yes
123456 -> no
"""
# Вариант №1:
s = int(input("Введите шестизначное число: "))
print((s // 100000 + s // 10000 % 10 + s // 1000 % 10)
      == (s % 1000 // 100 + s % 100 // 10 + s % 10))

# Вариант №2:
t = input('Введите номер билета: ')
l = int(t[0]) + int(t[1]) + int(t[2])
r = int(t[3]) + int(t[4]) + int(t[5])
if l == r:
    print('Yes')
else:
    print('NO')

# Вариант №3:
s = input('Введите 6-значный номер билета: ')
if len(s) != 6:
    print(f'Число {s} не 6-ти значное')
else:
    sum1 = 0
    sum2 = 0
    for i in range(len(s)//2):
        sum1 += int(s[i])
        sum2 += int(s[len(s)//2 + i])
    if sum1 == sum2:
        print(f'{s} Cчастливый номер билета')
    else:
        print(f'{s} Не счастливый номер билета, повезёт в следующий раз! :)')
