# Задача 38:
# Дополнить телефонный справочник c возможностью изменения и удаления данных.
# Пользователь также может ввести имя или фамилию.
# Вы должны реализовать функционал для изменения и удаления данных.

#       Декомпозиция:
# 1.  Ввод. Выбор действий (choose_action) из телефонной книги пользователем с клавиатуры;
# 2.  Метод импорта данных (import_data) в формате .txt;
# 3.  Метод чтение файла (read_file_to_dict) для словаря контактов (фамилия, имя, телефон);
# 4.  Метод чтение файла (read_file_to_list) для создания списка контактов (фамилия, имя, телефон);
# 5.  Метод для поиска параметров (search_parameters) с помощью выбора пользователем;
# 6.  Метод нахождения номера телефона (find_number) в списке контактов;
# 7.  Метод получения нового номера (get_new_number)с клавиатуры пользователем;
# 8.  Метод добавления номера телефона (add_phone_number);
# 9.  Метод показа телефонной книги (show_phonebook);
# 10. Метод поиска для изменения (search_to_modify);
# 11. Метод изменения номера телефона (change_phone_number);
# 12. Метод удаления контакта (delete_contact);
# 13. Метод печати контактов (print_contacts).
